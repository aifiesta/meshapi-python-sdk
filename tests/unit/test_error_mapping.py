...
def test_401_unauthorized():
    ...
    err = MeshAPIError.from_response(resp)
...
def test_stream_interrupted_factory():
    err = MeshAPIError.stream_interrupted("connection reset")
...
def test_repr():
    err = MeshAPIError("oops", status=404, error_code="not_found", request_id="req_x")
    r = repr(err)
    assert "MeshAPIError" in r
    assert "404" in r
    assert "not_found" in r


def test_isinstance_exception():
    err = MeshAPIError("msg", status=400, error_code="bad_request", request_id="req_y")
